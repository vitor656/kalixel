...
class FlxTiledLoader
{
	...
	public static function loadLevel(state:FlxState, level:String)
	{
		...
		for (layer in tilesLayers)
		{
			...
			if (tiledLayer != null)
			{
				layerMap.loadMapFromArray(tiledLayer.tileArray, tiledMap.width, tiledMap.height, layer.tileSet, tiledMap.tileWidth, tiledMap.tileHeight,
					layer.autotile, 1);
